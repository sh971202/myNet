...
pthName = 'SP_local&class&weight_CN_'
...
def lossFunctionOri(input, target, quaternion, quaterniongt, tvec, tvecgt,\
        ...
        loss = (localizationLoss * (epochNum - epoch) + classficationLoss *
        epoch) / epochNum 
...
def train(args, myNet):
        ...
        for epochIdx, epoch in enumerate(range(epochNum)):
                ...
                for batchIdx, batch in enumerate(dataLoader):
                        ...
                        objPts = np.ascontiguousarray(corre[:, 2:, :])
                        imgPts = np.ascontiguousarray(corre[:, :2, :])
                        ransacobjPts = np.ascontiguousarray(ransacCorre[:, 2:, :])
...
def test(args, myNet):
        ...
        for batchIdx, batch in enumerate(dataLoader):
                ...
                (corre, label, ransacLabel, focalLength, quaternion, tvec) = batch
                corre = corre.squeeze()
                label = label.squeeze()
                ransacLabel = ransacLabel.squeeze()
                corre = Variable(corre.float())
