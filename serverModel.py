...
class SpNet(nn.Module):
	...
	def forward(self, x, dis):
		...
		buf = out
		newout = out.expand(x.size()[0], 32, 1, 8).clone()
		for correIdx, corre in enumerate(buf):
			for idx in range(7):
				...
				#print (corre.size())
			newout[correIdx] = corre
		newout = self.layer(newout)
		newout = self.conv2(newout)
		#print ('After conv2', newout.size())
		newout = self.finalConv(newout)
		#print ('After finalConv', newout.size())
		newout = newout.view(newout.size(0), -1)
		#print ('After view', newout.size())
		w = torch.tanh(newout)
		w = F.relu(w)
